...
// Used to store the promise for promoting pawns
let _promote;
...
async function clickPiece(event) {
    // If the game has not started or is paused, just return without doing anything
    ...
    // If there is a selected piece and if the clicked tile is a valid move for that piece
    if (pieceObject.getSelected !== null && pieceObject.getSelected.getValidMove([x_true, y_true])) {
        // For capturing a piece
        if ((pieceObject.getSelected.getColor === "white" && boardObject.pieceArrayPosition([x_true, y_true]) <= -1) 
            || (pieceObject.getSelected.getColor === "black" && boardObject.pieceArrayPosition([x_true, y_true]) >= 1)) {
        ...
        // For castling
        // Need to check for moved piece because I'm stupid and did castling in two different ways...
        ...
        for (let name in listObject.getPieceList) {
            ...
            listObject.getPieceList[name].getTaken);
        }

        // Updates the previous positions of pawns, but also includes promotion
        for (let name in listObject.getPawnList) {
            listObject.getPawnList[name].updatePreviousPosition(listObject.getPawnList[name].getMoved, 
            listObject.getPawnList[name].getTaken, listObject.getPawnList[name].getPromoted);
        }
